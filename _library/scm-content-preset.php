<?php
	...
	if ( ! function_exists( 'scm_content_preset_size' ) ) {
        function scm_content_preset_size( $size, $units, $fall = '', $fall2 = 'px' ) {
	    	...
	    	$size = ifexists( $size, $fall );
	 		$size = ( is_numeric( $size ) ? $size . $units : ifequal( $size, [ 'auto', 'initial', 'inherit' ], $fall ) );
	...
	if ( ! function_exists( 'scm_content_preset_rgba' ) ) {
        function scm_content_preset_rgba( $color, $alpha, $fall = '', $fall2 = 1 ) {
	    	...
	    	$color = is( $color, $fall );
	 		$color = ifequal( $color, [ '', 'transparent', 'initial', 'inherit', 'none' ], hex2rgba( $color, $alpha ) );
        ...
        function scm_content_preset_marker( $luogo, $fields = [], $mark = 0 ) {
			...
			switch ( $marker ) {
				...
				default:
					...
					$term_field = ( $term && sizeof( $term ) ? get_fields( $term[0] ) : [] );
