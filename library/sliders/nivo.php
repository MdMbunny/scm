...
*/
	...
	*/
	...
	$setNivoDefaults = array(
		...
		'prevText'                => '"' . __( '&laquo; Precedente', SCM_THEME ) . '"', // previous direction text
		'nextText'                => '"' . __( 'Successivo &raquo;', SCM_THEME ) . '"', // next direction text
		//buttons nav
				...
				if ( $slides->have_posts() ) {
					...
					while ( $slides->have_posts() ) {
						...
						if ( has_post_thumbnail() ) {
							...
							$image       = get_the_post_thumbnail( get_the_ID(), $imageSize, array( 'class' => 'img-featured slide', 'title' => esc_attr( '#nivo-slider-caption-' . $i ), 'data-style' => wm_css_background_meta( 'slide-' ) ) );
					...
					while ( $slides->have_posts() ) {
						...
						$link   = ( $link ) ? ( '<a href="' . $link . '" class="nivo-slide-link"' . $target . '></a>' ) : ( '' );
